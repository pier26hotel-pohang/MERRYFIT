    ...
    <main className="pt-8">
      ...
      <section className="mt-3 mb-4 rounded-2xl bg-emerald-800 p-5 text-white">
        ...
        <div className="text-sm text-emerald-100">
          {BRANCH_LABEL[member.branch] ?? member.branch} · {member.phone || "연락처 없음"} · {tierFor(attendedCount(db, id)).name} 등급
        </div>
        <div className="mt-3 grid grid-cols-3 gap-2 text-center">
          ...
          <div className="rounded-xl bg-emerald-700/60 py-2">
            ...
            <div className="text-[10px] text-emerald-200">1회 {tierFor(attendedCount(db, id)).point.toLocaleString()}원</div>
          </div>
        </div>
      </section>
